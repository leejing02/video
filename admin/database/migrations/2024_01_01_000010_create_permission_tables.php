...
// Spatie laravel-permission 标准迁移（精简版）
    ...
    public function up(): void
    {
        ...
        Schema::create($tableNames['model_has_permissions'], function (Blueprint $table) use ($tableNames, $columnNames, $pivotPerm) {
            ...
            $table->foreign($pivotPerm)
                ...
                ->onDelete('cascade');
            $table->primary([$pivotPerm, $columnNames['model_morph_key'], 'model_type'], 'model_has_permissions_pk');
        });
        ...
        Schema::create($tableNames['model_has_roles'], function (Blueprint $table) use ($tableNames, $columnNames, $pivotRole) {
            ...
            $table->foreign($pivotRole)
                ...
                ->onDelete('cascade');
            $table->primary([$pivotRole, $columnNames['model_morph_key'], 'model_type'], 'model_has_roles_pk');
        });
        ...
        Schema::create($tableNames['role_has_permissions'], function (Blueprint $table) use ($tableNames, $pivotRole, $pivotPerm) {
            ...
            $table->foreign($pivotRole)
                ->references('id')->on($tableNames['roles'])
                ->onDelete('cascade');
            $table->primary([$pivotPerm, $pivotRole], 'role_has_permissions_pk');
        });
